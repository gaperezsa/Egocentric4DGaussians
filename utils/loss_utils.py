...
def masked_ssim(img1, img2, mask, window_size=11):
    """Compute SSIM loss (1 - SSIM) averaged only over pixels where mask is True.

    The standard _ssim helper does not expose the per-pixel SSIM map, so we
    replicate the convolution-based computation here to get [B, C, H, W] then
    apply the spatial boolean mask before taking the mean.

    Args:
        img1, img2: [B, C, H, W] float tensors in [0, 1]
        mask:  [B, C, H, W] bool tensor — True means *include* in the loss.
               If None, falls back to a plain mean (identical to ``1 - ssim()``).
    Returns:
        Scalar: 1 - mean_ssim over masked pixels.
    """
    channel = img1.size(-3)
    ...
    # ---- Inline SSIM map computation ([B, C, H, W]) ----
    ...
    if mask is not None:
        ...
        return 1.0 - (ssim_map * mask_float).sum() / valid_count
    else:
